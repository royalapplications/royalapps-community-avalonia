// Adapted from royalapplications/royalapps-community-rdp/scripts/generate-api-docs.mjs.
// Copyright (c) 2023 Royal Apps. MIT license: generate-api-docs.LICENSE.
...
const projects = [
  { name: "RoyalApps.Community.Avalonia.Common", framework: "net10.0" },
  { name: "RoyalApps.Community.Avalonia.Windows", framework: "net10.0-windows" }
].map(project => ({
  ...project,
  sourceRoot: path.join(repoRoot, "src", project.name),
  projectFile: path.join(repoRoot, "src", project.name, `${project.name}.csproj`),
  xmlDocFile: path.join(repoRoot, "src", project.name, "bin", "Release", project.framework, `${project.name}.xml`)
}));
const documentedNamespacePrefix = "RoyalApps.Community.Avalonia";
const publicApi = collectPublicApiSurface(projects.map(project => project.sourceRoot));
let memberDocs = new Map();
let resolvedMemberDocs = new Map();

function generateApiDocs() {
  const args = process.argv.slice(2);
  if (args.some(arg => arg !== "--no-restore")) {
    throw new Error("Usage: node scripts/generate-api-docs.mjs [--no-restore]");
  }
  // Build every project before replacing any generated documentation.
  for (const project of projects) {
    execFileSync("dotnet", [
      "build", project.projectFile, "-c", "Release", "-nologo",
      "-p:UseSharedCompilation=false", "-p:EnableWindowsTargeting=true",
      "-p:UsedAvaloniaProducts=", ...args
    ], { cwd: repoRoot, stdio: "inherit" });
  }
  const members = projects.flatMap(project => {
    const xml = readFileSync(project.xmlDocFile, "utf8");
    return Array.from(xml.matchAll(/<member name="([^"]+)">([\s\S]*?)<\/member>/g))
      .map(match => ({ id: match[1], body: match[2] }));
  });
  memberDocs = new Map(members.map(member => [member.id, parseMemberBody(member.id, member.body)]));
  resolvedMemberDocs = new Map();
  ...
  for (const typeMember of typeMembers) {
    ...
    const simpleTypeName = getDisplayTypeName(fullTypeName);
    ...
    pages.set(pagePath, renderTypePage(fullTypeName, simpleTypeName, typeMember.id, linkedMembers, resolvedMemberDocs, publicApi));
  ...
  for (const entry of readdirSync(generatedRoot, { withFileTypes: true })) {
    ...
    if (entry.isFile() && entry.name.endsWith(".md") && !pages.has(file)) rmSync(file);
  }
  for (const [file, content] of pages) writeFileSync(file, content);
  ...
  console.log(`Generated ${pages.size} public API pages from ${projects.length} libraries.`);
}
...
export { collectPublicApiSurface, getDisplayTypeName, renderTypeExpression };
...
function renderCref(cref) {
  ...
  if (kind === "T" && publicApi.publicTypes.has(normalizedCref.slice(2))) {
    return renderTypeLink(normalizedCref.slice(2));
  }
...
function renderTypePage(fullTypeName, simpleTypeName, typeMemberId, linkedMembers, docsByMember, publicApiSurface) {
  ...
  const lines = [
    ...
    "",
    "```csharp",
    typeInfo?.declaration ?? fullTypeName,
    "```",
    "",
    `Namespace: \`${typeInfo?.namespaceName}\``,
    "",
    `[View source](https://github.com/royalapplications/royalapps-community-avalonia/blob/main/${typeInfo?.sourceFile})`
  ];
...
function formatMemberHeading(memberId, itemMetadata, itemDocs, publicApiSurface) {
  ...
  const externalGlyph = externalLink
    ? ` <a class="api-member-external-link" href="${externalLink}" target="_blank" rel="noreferrer" title="Open external reference">â†—</a>`
    : "";
...
function simplifyTypeName(typeName) {
  return typeName
    ...
    .replace(/RoyalApps\.Community\.Avalonia\./g, "")
    .replace(/\{/g, "<")
    .replace(/\}/g, ">");
}
...
function getDisplayTypeName(fullTypeName) {
  const simpleName = fullTypeName.slice(fullTypeName.lastIndexOf(".") + 1).replace(/`[0-9]+/g, "");
  const parameters = publicApi.typeInfos.get(fullTypeName)?.typeParameters ?? [];
  return parameters.length ? `${simpleName}<${parameters.join(", ")}>` : simpleName;
}

function renderTypeLink(fullTypeName) {
  return `[${getDisplayTypeName(fullTypeName).replace(/</g, "&lt;").replace(/>/g, "&gt;")}](${getTypeDocLink(fullTypeName)})`;
}

function renderApiIndex() {
  return "---\neditLink: false\n---\n# API reference\n\nGenerated from the Common and Windows libraries' compiler XML documentation and C# source. Use the sidebar to browse namespaces and types. Framework-inherited members and internal implementation types are omitted.\n\nUpdate the XML comments in `src/` and run `npm run docs:api` to refresh this reference.\n";
}

function renderSidebarModule(namespaces, pagesByNamespace) {
  const groups = projects.map(project => ({
    text: project.name.replace(`${documentedNamespacePrefix}.`, ""),
    collapsed: false,
    items: namespaces.filter(name => name === project.name || name.startsWith(`${project.name}.`)).map(namespaceName => ({
      text: namespaceName === project.name ? "Core" : namespaceName.slice(project.name.length + 1),
      collapsed: false,
      items: pagesByNamespace.get(namespaceName).map(entry => ({
        text: entry.simpleTypeName.replace(/</g, "&lt;").replace(/>/g, "&gt;"),
        link: getTypeDocLink(entry.fullTypeName)
      }))
    }))
  }));
...
function compareTypeEntries(left, right) {
  return left.simpleTypeName.localeCompare(right.simpleTypeName, "en", { numeric: true });
}
...
function collectPublicApiSurface(sourceRoots) {
  ...
  const files = (Array.isArray(sourceRoots) ? sourceRoots : [sourceRoots]).flatMap(enumerateSourceFiles).sort();
  for (const filePath of files) {
    ...
    const lines = joinDeclarationLines(content.split(/\r?\n/));
    let braceDepth = 0;
    ...
    for (const line of lines) {
      ...
      if (!currentType && publicTypeMatch) {
        const typeParameters = publicTypeMatch[3]?.split(",").map(value => value.trim()) ?? [];
        const arity = typeParameters.length ? `\`${typeParameters.length}` : "";
        const fullTypeName = `${namespaceName}.${publicTypeMatch[2]}${arity}`;
        ...
        currentType = {
          ...
          relatedTypeReferences: splitTypeReferences(publicTypeMatch[4]),
          nextEnumValue: 0
        };
      ...
      else if (currentType && braceDepth === currentType.declarationDepth && shouldParseMember(trimmed, currentType.kind)) {
...
function joinDeclarationLines(lines) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const trimmed = line.trim();
    if (!trimmed.startsWith("//") && /^(public|protected|event)\s+/.test(trimmed) &&
        !/\b(class|interface|struct|enum)\s+/.test(trimmed)) {
      while (!/[{;=]/.test(line) && index + 1 < lines.length) {
        line += " " + lines[++index].trim();
      }
    }
    result.push(line.trim().startsWith("//") ? "" : line);
  }
  return result;
}
...
function tryParseMemberDeclaration(trimmedLine, simpleTypeName) {
  ...
  const normalizedLine = trimmedLine
    ...
    .replace(/^(?:(?:static|virtual|override|abstract|sealed|partial|new|extern|unsafe|readonly|const)\s+)+/, "");

  const fieldMatch = normalizedLine.match(/^([^=()]+?)\s+(\w+)\s*=(?!=|>)/);
  if (fieldMatch) {
    return { key: fieldMatch[2], kind: "field", name: fieldMatch[2], type: fieldMatch[1].trim() };
  }
...
function shouldParseMember(trimmedLine, typeKind) {
  ...
  return trimmedLine.startsWith("public ") ||
    (trimmedLine.startsWith("protected ") && !/\b(?:override|private|internal)\b/.test(trimmedLine));
}
...
function renderTypeToken(token, publicApiSurface) {
  ...
  const simplifiedToken = token
    ...
    .replace(/^RoyalApps\.Community\.Avalonia\./, "");
